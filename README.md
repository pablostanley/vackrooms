...
- `src/lib/game/computer-crt.ts` and `src/shaders/crt.wgsl`: vgpu renders shared static curvature and glass maps once, then releases its GPU device. Native SVG curves the browser chrome; a glass layer adds restrained scanlines, warm phosphor color, and vignette over the live page. No website pixels are captured and no extra per-frame readback is needed. WebGPU failure keeps the existing CSS glass.
